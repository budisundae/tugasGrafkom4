...
void Demo::Render() {
	...
	DrawKulkasDepan(this->depthmapShader);
	DrawKulkasSamping(this->depthmapShader);
	DrawKulkasAtas(this->depthmapShader);
	...
	glm::vec3 cameraPos = glm::vec3(2, 5, 3);
	...
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, depthMap);
	DrawKulkasDepan(this->shadowmapShader);
	...
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, depthMap);
	DrawKulkasSamping(this->shadowmapShader);
	...
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, depthMap);
	DrawKulkasAtas(this->shadowmapShader);
...
void Demo::BuildKulkasDepan()
{
	...
	float vertices[] = {
		...
		// front
		-0.5, -1.5, 0.5, 0, 0, 0.0f,  0.0f,  1.0f, // 0
		0.5, -1.5, 0.5, 1, 0,  0.0f,  0.0f,  1.0f, // 1
		0.5,  1.5, 0.5, 1, 1,  0.0f,  0.0f,  1.0f, // 2
		-0.5,  1.5, 0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 3
...
void Demo::BuildKulkasSamping()
{
	...
	float vertices[] = {
		...
		-0.5,  0.5, 0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 3
		 ...
		 0.5,  1.5, -0.5, 1, 0, 1.0f,  0.0f,  0.0f, // 5
		 0.5, -1.5, -0.5, 1, 1, 1.0f,  0.0f,  0.0f, // 6
		 0.5, -1.5,  0.5, 0, 1, 1.0f,  0.0f,  0.0f, // 7

		// back
		-0.5, -1.5, -0.5, 0, 0, 0.0f,  0.0f,  -1.0f, // 8 
		0.5,  -1.5, -0.5, 1, 0, 0.0f,  0.0f,  -1.0f, // 9
		0.5,   1.5, -0.5, 1, 1, 0.0f,  0.0f,  -1.0f, // 10
		-0.5,  1.5, -0.5, 0, 1, 0.0f,  0.0f,  -1.0f, // 11

		 // left
		 -0.5, -1.5, -0.5, 0, 0, -1.0f,  0.0f,  0.0f, // 12
		 -0.5, -1.5,  0.5, 1, 0, -1.0f,  0.0f,  0.0f, // 13
		 -0.5,  1.5,  0.5, 1, 1, -1.0f,  0.0f,  0.0f, // 14
		 -0.5,  1.5, -0.5, 0, 1, -1.0f,  0.0f,  0.0f, // 15
...
void Demo::BuildKulkasAtas()
{
	...
	float vertices[] = {
		...
		-0.5, 1.5,  0.5, 1, 0,   0.0f,  1.0f,  0.0f, // 17
		-0.5, 1.5, -0.5, 1, 1,  0.0f,  1.0f,  0.0f, // 18
		0.5, 1.5, -0.5, 0, 1,   0.0f,  1.0f,  0.0f, // 19
...
void Demo::DrawKulkasDepan(GLuint shader)
{
	UseShader(shader);
	glBindVertexArray(cubeVAO);
	glm::mat4 model;
...
void Demo::DrawKulkasSamping(GLuint shader)
{
	UseShader(shader);
	glBindVertexArray(cubeVAO2);
	glm::mat4 model;
...
void Demo::DrawKulkasAtas(GLuint shader)
{
	UseShader(shader);
	glBindVertexArray(cubeVAO3);
	glm::mat4 model;
